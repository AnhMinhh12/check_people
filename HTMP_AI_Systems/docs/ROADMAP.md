...
> **Phiên bản hiện tại**: V5.6 (Enterprise Edition - Optimized ONNX)  
> **Ngày cập nhật**: 08/04/2026  
> **Trạng thái**: Kiến trúc HTMP_AI_Systems hoàn chỉnh — Multi-Camera WorkerManager, ONNX Runtime, Repository Pattern.
...
|---|---|---|
| Chạy trên CPU (không có GPU) | 🟡 Trung bình | FPS bị giới hạn ở mức 1-5 tùy thuộc vào số lượng camera |
...
- **Docker Swarm / Kubernetes**: Mỗi camera là một Container Docker riêng biệt.
  ...
  services:
    warden-cam01:
      image: ghcr.io/dunghq23/htmp_ai_systems:dev-anhminh
      environment:
        - CAMERA_ID=cam_01
...
### Phase 2 — Tái cấu trúc Enterprise (Đã hoàn thành ✅)
- [x] Tích hợp EventDispatcher (events/dispatcher.py) & Module tích hợp (integrations/)
- [ ] Chuyển Database sang PostgreSQL (khi > 20 camera)
...
- [ ] Kích hoạt TelegramNotifier (integrations/telegram.py — đã có skeleton)
- [ ] Kích hoạt ERPConnector (integrations/erp.py — đã có skeleton)
...
Dự án **Sentinel Warden AI** đã hoàn thiện nền tảng Enterprise V5.5 với kiến trúc HTMP_AI_Systems modular, SQLAlchemy ORM, Repository Pattern, Docker hóa và CI/CD tự động. Skeleton tích hợp Telegram và ERP đã có sẵn, chỉ cần kích hoạt cấu hình. Mỗi giai đoạn mở rộng sẽ được xây dựng "chồng lên" nền tảng hiện có.

**Ưu tiên cần làm ngay**:
1. Kết nối LAN cho Camera (thay WiFi)
2. Kích hoạt Telegram Bot (integrations/telegram.py đã sẵn sàng)
3. Hoàn thiện tab Phân Tích với dữ liệu thực từ DB
4. Mua GPU để nâng FPS

---
*Tài liệu được cập nhật lần cuối: 08/04/2026 — Sentinel Warden AI Team*
